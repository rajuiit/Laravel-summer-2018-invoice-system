...
@section('return')
    ...
    <div class="block-header">
        <h2>Return back your product</h2>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-6">
                <form class="editingCartForm" action="/saveChanges"  method="post"  >
                    ...
                    <div class="selling-right-side">
                        ...
                        <select disabled onchange="nameTracker()"  name="customerName" class="form-control cus-name-select"  >
                            ...
                            @foreach($seller as $varThree)
                                <option {{ ( $varThree->name == $sellerName ) ? 'selected' : '' }} value="{{$varThree->name}}">{{$varThree->name}}</option>
                        @endforeach
                        <!-- This hidden input is only for fetching selected customer info while page loading ) It is allowed only for this page -->
                            <input name="selected-customer" class="selectedCustomer" type="hidden" value="{{$sellerName}}">
                        </select>
                    </div>
                    ...
                    <p>My Bill</p>
                    <div class="dismiss-spin">
                        <input  name="myBill" type="number" class="form-control  myBill" placeholder="Payment Taking" min="0" value="{{$myBill}}" >
                    </div>

                    <br>
                    <p>Paid Before</p>
                    <div class="dismiss-spin">
                        <input disabled name="paid" type="number" class="form-control  " placeholder="Payment Taking" min="0" value="{{$payment}}" >
                    </div>
 <br>
                    <p>Sellers Voucher No</p>
                    <div class="dismiss-spin">
                        <input  name="sellersVoucher" type="text" class="form-control "  min="0" value="{{$sellersVoucher}}" >
                    </div>
...
@section('sellScript')
    ...
    <script>
        ...
        $(".cart-btn").click(function () {
            ...
            $.ajax({
                type:'get',
                url:'/MyReturnCartDataFetch/'+voucherNo+'',
                success:function (response) {
        ...
        $(".data-send").click(function () {
            //how much taka client paying to us it is here ...............
            var name = $( ".cus-name-select option:selected" ).val();
            var voucher = $(".returnsVoucherNo").val();
            var bill = $(".myBill").val();
            var paying = $(".payment-taking ").val();
            ...
            $.ajax({
                type:'get',
                url:'/cusInfoTrackingAfterClickingDataSendforBrought/'+name+'/'+voucher+'/'+bill+'/'+paying+'/'+discount+' ',
                success:function (response) {
                   $(".priceDewResult").html(response);
                }
            })
        ...
        $(window).load(function(){
            ...
            $.ajax({
                type:'get',
                url:'/discountCounterForBroughtReturn/'+name+'/'+paying+'/'+voucherNo+'/'+myBill+' ',
                success:function (response) {
                    $(".priceDewResult").html(response);
                }
            })
        ...
        $(".myBill, .payment-taking").keyup(function () {
            //how much taka client paying to us it is here ...............
            var name = $( ".cus-name-select option:selected" ).val();
            var myBill = $(".myBill").val();
            var paying = $(".payment-taking").val();
